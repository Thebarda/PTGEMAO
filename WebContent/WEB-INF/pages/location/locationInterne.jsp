					...
					<fieldset>
						...
						<c:choose>
							...
							<c:otherwise>
								...
								<div>
									<span class='text-label'>Catégorie : </span><span><%=session.getAttribute("nomCategorie")%></span>
								</div>
					</fieldset>
					<fieldset>
						<br> <label for="instrument">Instrument : </label> <br>
						<table class="tablesorter-blue  pure-table">
							<thead>
								<tr>
									...
									<th>date d'achat</th>
									<th>valeur achat</th>
									<th>valeur reapprovisionnement</th>
									<th>est déplacable</th>
									<th>observation</th>
									<th>Etat</th>
									<th>Choix</th>
								</tr>
							...
							<c:forEach items="${listeMateriel}" var="instrument">
								<tr>
									...
									<td><input type="radio" name="nomDesignation"
										value="${instrument.key }" checked>Choisir</td>
								</tr>
							</c:forEach>
						</table>
						<br> <label for="adh">Adhérent : </label><input id="adh"
							type="text" name="adherent"><br>
						<br> <label>Date de la location : </label><input
							id="dateEmprunt" type="text" class="datepicker"
							required='required' name="debutLocation"> <br>
						<br>
						<label>Date encaissement : TODO</label>
					</fieldset>
					</c:otherwise>
		</c:choose>
		<fieldset class='align-center no-border'>
			<a href="<c:url value="<%=Pattern.ACCUEIL%>"/>">Annuler</a> <input
				id="valider" type="submit" value="Valider" />
		</fieldset>
		</form>
	</c:when>
	<c:otherwise>
		<form method="post" action="#">
			<fieldset>
				...
				<br> <label>Adhérent : </label><span id="nomAdherent"><%=session.getAttribute("nomAdherent")%></span><br>
				...
				<label>Montant : </label><span id="leMontant"><%=session.getAttribute("montant")%>
					euros par an</span><br> <label>Caution : </label><span id="laCaution"><%=session.getAttribute("caution")%>
					euros</span><br> <label>Imprimer : </label><input type="radio"
					name="imprimer" value="Oui" checked> Oui <input
					type="radio" name="imprimer" value="Non"> Non<br>
			</fieldset>
			<fieldset class='align-center no-border'>
				<a href="<c:url value="<%=Pattern.ACCUEIL%>"/>">Annuler</a> <input
...
<c:otherwise>
	<p class="offset text-success"><%=request.getAttribute("validation")%></p>
	<br>
